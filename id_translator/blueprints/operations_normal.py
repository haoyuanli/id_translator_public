"""
Methods to handle incoming requests - Uses db_handler
...
from pymongo.errors import ConnectionFailure, BulkWriteError, ServerSelectionTimeoutError, InvalidName

from id_translator.api.nav import nav
...
def upload():
    """
    Route to upload page. Buttons and form utilize ../static/js/uploader_normal.js
    ...
    for this page, the uploader_normal.js is replacing the boolean 
    ...
    if form.upload_confirm.data:
        ...
        try:
            ...
            status, new = APP.config["MongoHandler"].update_collection(file_path)

            if status == 206:
                missed_msg = "No records were added to the database. Check source file."

                payload = json.dumps({"missed": missed_msg})
            ...
            return Response("Database Write Error: {}".format(BulkWriteError.details), status=500)
        except KeyError:
            return Response('{} is not the PK of this dataset!'.
                            format(APP.config['project']['PRIMARY_KEY']), status=404)
        except FileNotFoundError:
            return Response("{} was not able to be found. "
                            "Contact the ID Translator SysAdmin.".format(filename), status=404)

    return render_template('/b_upload_normal.html', form=form, table=table)
